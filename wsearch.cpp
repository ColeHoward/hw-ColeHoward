                    ...
                    Location delta,string word, unsigned int currWordIndex)
{
    ...
    if(grid[currLoc.row + delta.row][currLoc.col + delta.col] == word[currWordIndex]){
       
        currLoc.row += delta.row;
        currLoc.col += delta.col;
        
        //if the last letter matched, the word must've been found
